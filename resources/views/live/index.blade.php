            ...
            <div class="col-xl-5">
                <div class="panel profile-panel" id="profilePanel">
                    ...
                    <div class="employee-name" id="employee_name">
                        @if( $attendance ) {{ optional( $attendance->employee )->name ?? 'UNKNOWN EMPLOYEE' }} 
                            @else WAITING FOR SCAN
                        @endif
                    </div>
                    <div class="employee-position" id="employee_position">
                        @if( $attendance && $attendance->employee && $attendance->employee->position ) {{ $attendance ->employee ->position ->position_name }} 
                            @else Place your finger on the biometric device 
                        @endif
                    </div>
                    <div class="row g-3 employee-info">
                        <div class="col-6">
                            <div class="info-box">
                                ...
                                <div class="info-value" id="employee_no">
                                    @if( $attendance && $attendance->employee ) {{ $attendance ->employee ->employee_no ?? '------' }} @else ------ @endif
                                </div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="info-box">
                                ...
                                <div class="info-value" id="department">
                                    @if( $attendance && $attendance->employee ) {{ optional( $attendance ->employee ->department ) ->department_name ?? '------' }} 
                                    @else ------ @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="status-box" id="statusBox">
                        <div class="status-text" id="status">
                            @if( $attendance ) @if( $attendance->time_out )
                            <i class="bi bi-box-arrow-right">
                            </i> TIME OUT RECORDED 
                            @else
                            <i class="bi bi-check-circle-fill">
                            </i> TIME IN RECORDED @endif 
                            @else
                            <i class="bi bi-fingerprint">
                            </i> READY TO SCAN @endif
                        </div>
                        <div class="attendance-time" id="attendance_time">
                            @if( $attendance ) 
                            @if( $attendance->time_out ) {{ \Carbon\Carbon ::parse( $attendance->time_out )->format( 'h:i:s A' ) }} 
                            @elseif( $attendance->time_in ) {{ \Carbon\Carbon ::parse( $attendance->time_in )->format( 'h:i:s A' ) }} 
                            @else --:--:-- @endif
                            @else --:--:-- @endif
                        </div>
                    </div>
...
================================================== -->
    ...
    <script>
        ...
        function updateClock() {
            ...
            document.getElementById("date").innerHTML=now.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' } );
        }
        ...
        let savedTheme=localStorage.getItem("ntc-theme") || "dark";
        applyTheme(
            savedTheme
        );
        ...
        */
        ...
        response=> {
        if(!response.ok){
             throw new Error("Biometric sync failed");
        }
        return response.text();
        }).then(()=>{
        ...
        if(!response.ok){
        throw new Error("Cannot load live attendance");
    }
        return response.json();
    }).then(data=>{
